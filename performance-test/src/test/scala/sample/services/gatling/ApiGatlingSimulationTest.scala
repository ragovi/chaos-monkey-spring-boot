package sample.services.gatling
...
class ApiGatlingSimulationTest extends Simulation {
  ...
  val scn = scenario("AddAndFindOrders").repeat(20, "n") {
        exec(
          http("AddOrder-API")
            .post("http://localhost:8090/order-service/orders")
            .header("Content-Type", "application/json")
            .body(StringBody("""{"productId":""" + Random.nextInt(20) + ""","customerId":""" + Random.nextInt(20) + ""","productsCount":1,"status":"NEW"}"""))
